...
public class WebViewObject : MonoBehaviour
{
...
#else
        float offset = 20;
        float left = ((Screen.width - scale.x) / 2.0f + center.x) + offset;
        float right = (Screen.width - (left + scale.x)) - offset;
        float bottom = ((Screen.height - scale.y) / 2.0f + center.y) - offset;
        float top = (Screen.height - (bottom + scale.y)) + offet;
    ...
    public void SetMargins(int left, int top, int right, int bottom, bool relative = false)
    {
        ...
        _gree_unity_webview_setMargins(name, (int)ml + offset * 2, (int)mt + offset * 2, (int)mr + offset * 2, (int)mb - offset * 2);
#elif UNITY_EDITOR_OSX || UNITY_STANDALONE_OSX
